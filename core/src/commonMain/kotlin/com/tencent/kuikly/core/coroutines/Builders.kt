    ...
    suspendCoroutine<Unit> { cont ->
        ...
        ref = setTimeout(pagerId, timeMs) {
            try {
                ...
                if (aj != null && !aj.isActive) return@setTimeout
                ref?.let { aj?.unregisterTimeout(it) }
        ...
        ref?.let { aj?.registerTimeout(pagerId, it) }
    }
}

suspend fun <T> suspendCancellableBridge(block: (kotlin.coroutines.Continuation<T>, ((Throwable?) -> Unit) -> Unit) -> Unit): T = kotlin.coroutines.suspendCoroutine { cont ->
    ...
    val safe = object : kotlin.coroutines.Continuation<T> {
        ...
        override fun resumeWith(result: Result<T>) {
            val active = (context[Job] as? Job)?.isActive != false
            if (!active) return
