...
public class TMNestStageCardAttr : ComposeAttr() {
    ...
    // 外部悬浮内容的顶部位置
    var outerHoverTop: Float by observable(0f)

    // 外部悬浮内容
    var outerHoverContent: ViewBuilder? = null

    // 外部悬浮内容的 zIndex 值
    var outerHoverZIndex: Int = 0
    ...
    fun outerHoverContent(content: ViewBuilder) = apply { this.outerHoverContent = content }

    // 数值设置方法
    fun footerWidth(width: Float) = apply { this.footerWidth = width }
    fun innerHoverTop(top: Float) = apply { this.innerHoverTop = top }
    fun outerHoverTop(top: Float) = apply { this.outerHoverTop = top }
    fun outerHoverZIndex(zIndex: Int) = apply { this.outerHoverZIndex = zIndex }
...
public class TMNestStageCardView : ComposeView<TMNestStageCardAttr, TMNestStageCardViewEvent>() {
    ...
    var isScrollWithAnimBySystemApi by observable(false)
    var isIOSDragEndCallBack by observable(false)
    var systemScrollStartOffset by observable(0f)
...
//        TMLog.i(TAG,"[updateMaxCardAttr] curoffset:${curOuterScrollOffset} int:${curOuterScrollOffset.roundToInt()} maxOffset:${maxOffSet}")
    ...
    private fun updateOuterScrollState(offsetY: Float) {
        ...
        if (levelHeights.isNotEmpty() && levelHeights.size > 1) {
            ...
            val denominator = maxHeight - minHeight
            cachedHeightRatio =
                if (denominator != 0f) (cardHeight - minHeight) / denominator else 0f
        }
            ...
            View {
                ...
                Scroller {
                    ...
                    Hover {
                        attr {
                            absolutePosition(
                                top = ctx.attr.outerHoverTop +(if (ctx.baseConfigChangeFlag) ctx.getContentPaddingTop() else ctx.getContentPaddingTop()),
                                left = 0f,
                                right = 0f
                            )
                            zIndex(ctx.attr.outerHoverZIndex)
                        }
                        vif({ ctx.attr.outerHoverContent != null }) {
                            ctx.attr.outerHoverContent?.invoke(this)
                        }
                    }
                    // 上方空白区域
                    View {
                    ...
                    View {
                        attr {
                            border(Border(2f,BorderStyle.SOLID,Color.YELLOW))
                            borderRadius(
...
//                            )
                        ...
                        scrollEnd {
                            // iOS种快速滑动脱手后，会先回调一次scrollEnd，然后再调用系统api滚动时，第二次回调的scrollEnd才是正常值
                            if (ctx.isScrollWithAnimBySystemApi && ctx.pagerData.isIOS && !ctx.isIOSDragEndCallBack) {
                                ctx.isIOSDragEndCallBack = true
                                ctx.systemScrollStartOffset = it.offsetY
                                TMLog.i(
                                    TAG, "scroll-inner-Height ios dragEnd call Back"
                                )
                                return@scrollEnd
                            }
                            TMLog.i(
                                TAG,
                                "scroll-inner-Height outScroller【scrollEnd】滚动结束 offsetY:${it.offsetY}  isqualStartOffset=${it.offsetY == ctx.systemScrollStartOffset}"
                            )
                            ctx.isScrollWithAnimBySystemApi =
                                it.offsetY == ctx.systemScrollStartOffset
...
//                                )
                    ...
                    dragEnd { params ->
                        ...
                        val isMaxLevel = ctx.currentLevel == ctx.attr.levelHeights.size - 1
                        // 大页卡 + 开启刷新时，不重置滚动位置（避免打断下拉刷新）
                        val shouldResetOffset = !ctx.isChildFlingEnable && ctx.pagerData.isIOS && !(isMaxLevel && ctx.attr.enableHeaderRefresh)
                        if (shouldResetOffset) {
                            ctx.innerScrollView?.view?.setContentOffset(0f, 0f, true)
                        }
                        TMLog.i(TAG, "[dragEnd] isMaxLevel=$isMaxLevel, enableHeaderRefresh=${ctx.attr.enableHeaderRefresh}, shouldResetOffset=$shouldResetOffset")
...
//                                TMLog.i(TAG, "update listHeight stickContentHeight[layoutFrameDidChange]stick height:${frame.height}")
    ...
    private fun calcInnerScrollPlaceHolderHeight(): Float {
        if (isOutDragging || isScrollWithAnimBySystemApi) {
            return 0f
        }
        scrollContentHeight = getScrollContentHeightInner()
...
//        )
    ...
    /**
     * 封装外部滚动容器的滚动方法
     * 统一处理 abortContentOffsetAnimate、setContentOffset 和动画状态标记
     * @param offsetY 目标滚动偏移量
     * @param animated 是否使用动画
     * @param abortPreviousAnim 是否中断之前的动画，默认为 true
     */
    private fun scrollOuterTo(offsetY: Float, animated: Boolean, abortPreviousAnim: Boolean = true) {
        if (abortPreviousAnim) {
            outerScrollView?.view?.abortContentOffsetAnimate()
        }
        outerScrollView?.view?.setContentOffset(0f, offsetY, animated)
        if (animated) {
            systemScrollStartOffset = curOuterScrollOffset
            isIOSDragEndCallBack = false
            isScrollWithAnimBySystemApi = true
        }
    }
...
//        TMLog.i(TAG, "[getCurrentCardHeight] cardHeight:${cardHeight}")
    ...
    private fun getScrollContentHeightInner(): Float {
        ...
        val height =
            if (curOuterScrollOffset >= 0) attr.levelHeights.last() - stickContentHeightUse else attr.levelHeights.last()
//        TMLog.i(
//            TAG,
...
//                                TMLog.i(TAG,"update listHeight stickContentHeight:${stickContentHeight} first:${isFirstLoad}")
    ...
    private fun attachToStageByVelocity(velocity: Float) {
        ...
        scrollOuterTo(stageHeight - levelHeights[0], needAnim)
        if (!needAnim) {
    ...
    fun snapToLevel(level: Int, animated: Boolean = true): Boolean {
        ...
        currentLevel = level
        // 执行滚动动画
        scrollOuterTo(targetOffset, animated)
...
//        TMLog.i(TAG, "[calcScrollHeight] yScale:${yScale}  enable:${attr.enableDynamicMargin}")
    ...
    override fun viewDidLayout() {
        ...
        scrollOuterTo(initOffset, false, abortPreviousAnim = false)
