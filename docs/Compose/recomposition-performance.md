...
> **已修复场景**：框架组件（如 `CompositionLocalProvider`）被 builtin filter 过滤导致的 scope 丢失，已通过 scopeKeySnapshot 机制解决 —— 在 `traceEventStart` 时即捕获 scope 快照，子节点不再因父节点被 filter 而丢失 scope。
