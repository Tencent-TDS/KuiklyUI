...
TurboDisplay 默认加载页面的初始状态，但现已支持`恢复页面退出时状态`,如列表滚动位置、轮播图当前页等，详情可参考「页面状态恢复」章节。
...
KRNativeRender({
    Kuikly({
        ...
        turboDisplayConfig: {
            diffDOMMode: 0,      // 0 不支持结构变化 ，1 支持结构变化，默认
            diffViewMode: 1,     // 0 分阶段差异更新，1 同步阶段更新
...
> **完整示例参考：** `demo/src/commonMain/kotlin/com/tencent/kuikly/demo/pages/demo/TBDemoTest/`
...
页面正常渲染，首屏完成后 TurboDisplay 自动将当前界面内容保存到本地缓存。若开启了自动采集，页面从首屏到完全加载期间的变化也会被持续记录，确保缓存内容始终接近业务实际效果。
