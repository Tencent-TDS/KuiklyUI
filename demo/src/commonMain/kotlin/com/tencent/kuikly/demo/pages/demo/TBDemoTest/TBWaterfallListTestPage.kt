        ...
        return {
            ...
            WaterfallList {
                ref {
                    ...
                    if (ctx.extraCacheContent.keySet().size > 0) {
                        KLog.i("当前extraContent", ctx.extraCacheContent.toString())
                        var key = ctx.extraCacheContent.keySet().toList()[0].toString()
                        val listPropsValue = ctx.extraCacheContent.toMap().get(key)
                        if (listPropsValue != null && listPropsValue.toString() != "null") {
                            try {
                                ...
                                ctx.restoredFirstVisibleIndex = (listProps["firstVisibleIndex"] as? Number)?.toInt() ?: 0
                                ctx.restoredFirstVisibleOffset = (listProps["firstVisibleOffset"] as? Number)?.toFloat() ?: 0f
                                ctx.listColumns = (listProps["columns"] as Number)?.toInt() ?: 1
                            } catch (e: Exception) {
