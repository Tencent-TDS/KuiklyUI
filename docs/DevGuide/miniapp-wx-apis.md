...
从 `1.9.22` / `2.0.21` / `2.1.21` 版本起，所有 WX Module 封装放在独立模块 **`core-wx`** 里（仅 `js(IR)` target，Android/iOS 不会引入僵尸类）。接入步骤：

**步骤 1：gradle 依赖**（仅 `jsMain` 引入）
...
> ⚠️ `core-wx` **只声明 js(IR) target**，不产出 android/iOS 产物，因此必须放到 `jsMain` 而非 `commonMain`。不要把它加到 `BasePager` 等跨端公共类里，而是在**实际用到 WX API 的页面**（通常是 `@Page("XxxPage")` 注解的 Pager 子类）里使用。

**步骤 2：在需要用到 WX Module 的页面里一行代码注册**

`core-wx` 提供一个 `Pager` 扩展方法，在**使用页面**的 `createExternalModules()` 里调用一次，即可把 10 个 WX Module 全部注册到当前 Pager：

```kotlin
// 仅放在 demo/src/jsMain/.../wx/WXApiExamplePage.kt 这类“只在小程序平台运行”的页面里
...
该扩展方法内置了平台判定（`pageData.params.optString("is_miniprogram") == "1"`），**仅在小程序平台生效**，其它平台调用为空操作。

> 为什么不放到 `BasePager` 统一注册？`core-wx` 只针对 `js(IR)` target，放在跨端公共类里会导致 Android/iOS 平台找不到该 API；而且只有部分 Pager 真正会用到 WX Module，按需在使用页面里注册更轻量、更直观。
>
