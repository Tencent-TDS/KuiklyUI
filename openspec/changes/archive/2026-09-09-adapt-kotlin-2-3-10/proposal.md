> ⚠️ **阅读提示（两处口径差异）**
>
> 1. **本文档是早期方案**：描述的是"2.1.21 与 2.3.10 **并存**、靠 `-c settings` 切换"。
>    该方案**已被取代**——最终改为"**默认版本即 2.3.10、不共存**"（见 `tasks.md` §11）。
>    因此文中"零侵入""并行分支""Gradle wrapper 不动""2.1.21 零修改"等表述均已失效。
> 2. **本文档来源于内网仓库 `KuiklyUI-Mirror`**，其中 `core-gradle-plugin` 版本化方案、
>    `KuiklyKotlinBuildVar.kt`、`main_internal` 基线等**均为内网分支状态，外网分支未同步**。
>    外网实际改动见 `tasks.md` §11.9。
