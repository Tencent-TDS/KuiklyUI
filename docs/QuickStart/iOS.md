...
- 从 Xcode 27 起，iOS SDK 支持的最低部署目标提升为 **15.0**，因此运行前需要调整所有pod库的iOS Deployment Target 字段。
...
:::: tip 提示
若使用 Xcode 27 及以上版本，请参照上文「Xcode 27 及以上版本需要额外配置」一节，将宿主 App target 与 Pods 中各 target 的 iOS Deployment Target 均设为 15.0 或更高。
::::
