...
### Requirement: Vertical `ScrollState` containers SHALL fulfill bring-into-view requests by pixel scrolling

A vertical scroll container backed by `ScrollState` SHALL calculate the focused target's bounds relative to its viewport, compare them against the IME-adjusted visible region, and scroll by the minimum vertical delta needed to make the target fully visible.

#### Scenario: Android long form scrolls focused input above keyboard
- **GIVEN** an Android Compose page uses `verticalScroll(ScrollState)` for a long form, and a lower text input is partially hidden by the keyboard
- **WHEN** that input receives a bring-into-view request
- **THEN** the container SHALL animate vertical scrolling until the input is fully visible above the IME area

#### Scenario: iOS long form scrolls focused input above keyboard
- **GIVEN** an iOS Compose page uses `verticalScroll(ScrollState)` for a long form, and a lower text input is partially hidden by the keyboard
- **WHEN** that input receives a bring-into-view request
- **THEN** the container SHALL animate vertical scrolling until the input is fully visible above the IME area

#### Scenario: HarmonyOS long form scrolls focused input above keyboard
- **GIVEN** a HarmonyOS Compose page uses `verticalScroll(ScrollState)` for a long form, and a lower text input is partially hidden by the keyboard
- **WHEN** that input receives a bring-into-view request
- **THEN** the container SHALL animate vertical scrolling until the input is fully visible above the IME area
